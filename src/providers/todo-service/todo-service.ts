...
export class TodoServiceProvider {
  ...
  getTodos() {
    ...
    addCategory2Todo(item: any, category: any) {
        ...
        return new Promise(resolve => {
            ...
            // then on the response, it'll map the JSON data to a parsed JS object.
            // Next, we process the data and resolve the promise with the new data.
            this.http.put(this.environment.getURL() + 'api/todos/' + item.id + '/categories/rel/' + category, {})
                .map(res => res.json())
                .subscribe(data => {
                    // we've got back the raw data, now generate the core schedule data
                    // and save the data for later reference
    ...
    getTODOCategories(item: any) {
        ...
        return new Promise(resolve => {
            ...
            this.http.get(this.environment.getURL() + 'api/todos/' + item.id + '/categories')
                .map(res => res.json())
                .subscribe(data => {
                    // we've got back the raw data, now generate the core schedule data
                    // and save the data for later reference
